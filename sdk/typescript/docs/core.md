...
const response = await services.core.getBalance({ client: rawClient });
...
const response = await services.core.getInfo({ client: rawClient });
console.log(response.data);
```
...
const response = await services.core.postStatistics({
    client: rawClient,
    body: {
        from: '...',  // string
        to: '...',  // string
